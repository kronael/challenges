// ROTTEN: head and tail share one cache line (no _Alignas(64) padding).
// This is CORRECT — it delivers every message in order — but it is the trap
// version: because the producer writes `tail` and the consumer writes `head`
// on every operation, and both indices sit in the same 64-byte cache line,
// each write invalidates the other core's cached copy of that line (false
// sharing). The two threads serialise through the cache-coherence protocol and
// throughput drops 3–5x versus the padded golden version. A single-threaded or
// low-volume check passes; the bench exposes the slowdown.
#include <pthread.h>
#include <stdatomic.h>
#include <stdbool.h>
#include <stdint.h>
...
typedef struct {
    _Atomic uint64_t tail;  // producer writes
    _Atomic uint64_t head;  // consumer writes — SAME cache line as tail (the bug)
...
int main(int argc, char **argv) {
    ...
    if (argc == 2 && strcmp(argv[1], "stress") == 0) {
        uintptr_t tail_line = (uintptr_t)&rb->tail / 64;
        uintptr_t head_line = (uintptr_t)&rb->head / 64;
        if (tail_line == head_line) {
            fputs("FAIL: producer and consumer counters share a cache line\n", stderr);
            free(rb);
            return 1;
        }
    }
